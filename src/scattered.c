...
{
	scattered	*g;
	if (n <= 0) error_and_exit ("scattered_new", "n must be >= 1.", __FILE__, __LINE__);
	if (!x || !y || !z) error_and_exit ("scattered_new", "x, y and z must be not empty.", __FILE__, __LINE__);
	g = scattered_new_0 (n, x, y, z, NULL, NULL, NULL);
	if (!g) error_and_exit ("scattered_new", "failed to create scattered object.", __FILE__, __LINE__);
	return g;
}
...
{
	scattered	*g;
	if (n <= 0) error_and_exit ("scattered_new_full", "n must be >= 1.", __FILE__, __LINE__);
	if (!x || !y || !z) error_and_exit ("scattered_new_full", "x, y and z must be not empty.", __FILE__, __LINE__);
	g = scattered_new_0 (n, x, y, z, dx, dy, dz);
	if (!g) error_and_exit ("scattered_new_full", "failed to create scattered object.", __FILE__, __LINE__);
	return g;
}
...
{
	if (!g) error_and_exit ("scattered_get_nth", "scattered *g is empty.", __FILE__, __LINE__);
	if (n < 0 || n >= g->n) error_and_exit ("scattered_get_nth", "index invalid.", __FILE__, __LINE__);
